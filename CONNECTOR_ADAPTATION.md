...
DB2 LUW 的 JDBC URL 已绑定 database，SQL 对象定位使用 `schema.table`；UPSERT 使用 `MERGE ... USING (VALUES ...)`。类型映射要注意 `DECIMAL` 最大精度 31、`TIMESTAMP` 小数秒精度最高 12，以及 CLOB/BLOB/GRAPHIC/DBCLOB/XML。DB2 LUW 不原生支持 `TIMESTAMP WITH TIME ZONE`，遇到该类型应显式转换而不是静默丢失时区。
