...
public final class Db2TypeMapper implements JdbcTypeMapper {
    ...
    static final int MAX_DECIMAL_SCALE = 30;
    ...
    private static DecimalType decimal(int precision, int scale) {
        ...
        if (precision > MAX_DECIMAL_PRECISION) {
            safeScale = Math.max(0, safeScale - (precision - MAX_DECIMAL_PRECISION));
        }
        safeScale = Math.min(safeScale, Math.min(MAX_DECIMAL_SCALE, safePrecision));
    ...
    private static String decimalType(Column column) {
        ...
        if (originalPrecision > MAX_DECIMAL_PRECISION) {
            scale = Math.max(0, scale - (originalPrecision - MAX_DECIMAL_PRECISION));
        }
        scale = Math.max(0, Math.min(scale, Math.min(MAX_DECIMAL_SCALE, precision)));
        return "DECIMAL(" + precision + "," + scale + ")";
    }
