...
final class JdbcCreateTableSqlResolver {
    ...
    private static boolean isOceanBase(JdbcConnectionConfig config) {
        if (DatabaseIdentifier.OCEANBASE.equalsIgnoreCase(config.getDialect())) {
            return true;
        }
        return OceanBaseJdbcUrl.accepts(config.getUrl());
    }

    private static boolean isOracle(JdbcConnectionConfig config) {
        if (DatabaseIdentifier.ORACLE.equalsIgnoreCase(config.getDialect())) {
            return true;
        }
        return OracleJdbcUrl.accepts(config.getUrl());
    }

    private static boolean isSqlServer(JdbcConnectionConfig config) {
        if (DatabaseIdentifier.SQLSERVER.equalsIgnoreCase(config.getDialect())) {
            return true;
        }
        return SqlServerJdbcUrl.accepts(config.getUrl());
    }

    private static boolean isDb2(JdbcConnectionConfig config) {
        if (DatabaseIdentifier.DB2.equalsIgnoreCase(config.getDialect())) {
            return true;
        }
        return Db2JdbcUrl.accepts(config.getUrl());
    }
